...
export class BondsPolishTreasuryEmissionService {
  ...
  async updatePLtr() {
    ...
    if (!scrappedData.data) {
      ...
      await Promise.all([
        this.edoModel.findOneAndUpdate(
          { id: edo.symbol },
          { id: edo.symbol, firstYear: edo.percentage[0], margin: edo.percentage[1] },
          { new: true, upsert: true },
        ),
        this.coiModel.findOneAndUpdate(
          { id: coi.symbol },
          { id: coi.symbol, firstYear: coi.percentage[0], margin: coi.percentage[1] },
          { new: true, upsert: true },
        ),
        this.rosModel.findOneAndUpdate(
          { id: ros.symbol },
          { id: ros.symbol, firstYear: ros.percentage[0], margin: ros.percentage[1] },
          { new: true, upsert: true },
        ),
        this.rodModel.findOneAndUpdate(
          { id: rod.symbol },
          { id: rod.symbol, firstYear: rod.percentage[0], margin: rod.percentage[1] },
          { new: true, upsert: true },
        ),
        this.otsModel.findOneAndUpdate(
          { id: ots.symbol },
          { id: ots.symbol, rate: ots.percentage[0] },
          { new: true, upsert: true },
        ),
        this.tosModel.findOneAndUpdate(
          { id: tos.symbol },
          { id: tos.symbol, rate: tos.percentage[0] },
          { new: true, upsert: true },
        ),
      ]);
    } catch (error) {
