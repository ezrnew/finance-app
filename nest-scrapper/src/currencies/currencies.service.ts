...
export class CurrenciesService {
    ...
    async getCurrencyRate(from:CurrencyType, to:CurrencyType){
        if(from===to) return 1
        let result;
        const currencyRateDocument = await this.currencyRateModel.findOne({$or:[
            {currencyRateString:from+to},
            {currencyRateString:to+from},
        ]})
        if(!currencyRateDocument){
            result = await this.currenciesRateScrapper.getCurrencyRate(from,to)
            if(!result) {console.log("no scrapping result");return}
...
// @ts-ignore
    ...
    return currencyRateDocument.currencyRateString === from+to? currencyRateDocument.rate: 1/currencyRateDocument.rate
}
