...
export const AddPaymentModal = () => {
  ...
  const { currentAccount, currentPortfolioId } = useTypedSelector(
    (state) => state.portfolio,
  );
  const { setCurrentPortfolio, refetchPortfolioData: updatePortfolioData } =
    useActions();
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await server.addOperation(
      currentPortfolioId,
      currentAccount?.id || "",
      amount,
    );
      ...
      >
        <form onSubmit={submitForm} className="flex flex-col space-y-2">
          ...
          <div className="w-full flex flex-col">
            ...
            <p className="mx-auto">
              {formatCurrency("Pl-pl", currentAccount?.cash || 0, "PLN")}
            </p>
          </div>
          ...
          <div className="flex space-x-2 !mt-6">
            <Button className="">Make Operation</Button>{" "}
            <Button
              variant={"secondary"}
              className="w-full"
              type="button"
